        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressList" :key="index" @click="checkIndex=index" :class="{'check':checkIndex==index}">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">{{item.streetName}}</dd>
                      <dd class="tel">{{item.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" @click="addressDel(item.addressId)">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" href="javascript:;" :to="{path:'/order' , query:{addressId:addressIds}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal :mdShows='isShow' @close="closeModal()">
        <p slot="message">确定要删除地址吗？</p>
  
        <div slot="btnGroup">
          <a href="javaScript:;" class="btn btn--m" @click='deleteAddress()'>确认</a>
          <a href="javaScript:;" class="btn btn--m" @click='isShow=false'>关闭</a>
        </div>
      </modal>
  ...
  export default{
      ...
      methods:{
        ...
        addressDel(addressId){
          this.isShow=true;
          this.deleteAdressId=addressId;
          
        },
        deleteAddress(){
          axios.post('/users/addressDel',{
           addressId:this.deleteAdressId
           }).then((response)=>{
            let res=response.data;
            if(res.status==0){
              this.isShow=false;
